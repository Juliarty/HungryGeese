...
class AlexNetQEstimator(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.features = nn.Sequential(
            nn.Conv2d(12, 32, kernel_size=(3, 4), stride=1, padding=(1, 0), padding_mode='circular'),# (7, 11) -> (7, 8)
...
class OneLayerNetQEstimator(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.features = nn.Sequential(
            # nn.Conv2d(12, 12, kernel_size=1, stride=1, padding=0),      # (7, 11) -> (7, 11)
            # nn.ReLU(inplace=True),
            nn.Conv2d(12, 64, kernel_size=(3, 4), stride=(3, 4), padding=(1, 1), padding_mode='circular'),  # (7, 11) -> (3, 3)
            nn.ReLU(inplace=True),
            # nn.ReLU(inplace=True),
            # nn.Conv2d(64, 64, kernel_size=3, padding=1),
            # nn.ReLU(inplace=True),
